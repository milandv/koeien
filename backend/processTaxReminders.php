...
while ($instance_row = mysql_fetch_array($instance_qry)) {
	...
	$qry = mysql_query("SELECT user_id, hash, firstname, email, SUM(remaining) as tax_owed FROM commons_users JOIN commons_users_taxes_owed USING(user_id) WHERE instance_id=$instance_id GROUP BY 1");
	while ($row = mysql_fetch_array($qry)) {
		$user_id = $row['user_id'];
		$hash = $row['hash'];
		$firstname = $row['firstname'];
		$email = $row['email'];
		$tax_owed = number_format($row['tax_owed'],2);
...
Your Commons Taxes are due. You currently owe $tax_owed.

You can pay your taxes by visiting our friendly Commons Revenue Serice <a href='http://grazingcows.org/mytaxes.php?h=$hash'>Payment Center</a>.
...
<p>Your Commons Taxes are due. You currently owe $tax_owed.</p>

<p>You can pay your taxes by visiting our friendly <a href='http://grazingcows.org/mytaxes.php?h=$hash'>Commons Revenue Service Payment Center</a>.</p>
...
$msg = new Email($Recipient, $Sender, "Your Cow Commons Taxes Are Due! - day $commons_day");
