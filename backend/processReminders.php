...
while ($instance_row = mysql_fetch_array($instance_qry)) {
	...
	$Sender = "$name Reminder Alerts <info@grazingcows.org>";
	if ($include_weekends == 0) {
		//don't process on Saturday or Sunday
	...
	$qry = mysql_query("SELECT user_id, firstname, email FROM commons_users JOIN commons_users_alerts USING(user_id) WHERE instance_id=$instance_id");
	while ($row = mysql_fetch_array($qry)) {
		$user_id = $row['user_id'];
		$firstname = $row['firstname'];
		$email = $row['email'];
		
		$qry2 = mysql_query("SELECT herd_size, avg_health FROM commons_herds WHERE user_id=$user_id ORDER BY ts DESC LIMIT 1");
		while ($row2 = mysql_fetch_array($qry2)) {
			$herd_size = $row2['herd_size'];
			$avg_health = $row2['avg_health'];
		}

	$Recipient = $email;


$textVersion = "Dear farmer $firstname,

Here's your daily report about your cows.

Your herd currently has $herd_size cows. Their average health is $avg_health.

You have until midnight tonight to decide how many cows to graze tomorrow. To set the amount of cows for tomorrow go here:

http://grazingcows.org/login.php
";

$htmlVersion =  "<p>Dear farmer $firstname,</p>

<p>Here's your daily report about your cows.</p>

<p>Your herd currently has $herd_size cows. Their average health is $avg_health.</p>

<p>You have until midnight tonight to decide how many cows to graze tomorrow. To set the amount of cows for tomorrow go here:</p>

<p><a href='http://grazingcows.org/login.php'>http://grazingcows.org/login.php</a></p>
";

$msg = new Email($Recipient, $Sender, "Cow Reminder - day $commons_day");
